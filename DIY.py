...
class VCodeGenerator(object):
    ...
    '''
    def __init__(self, fontPath, numOfChars, height):
    ...
    def __generateRandChar(self):
        finalChar = ''
        firstRan = random.randint(65, 90);
        if (firstRan % 2):
            firstRan = random.randint(65, 90) % 10
            finalChar = str(firstRan)
        else:
            finalChar = chr(firstRan)
        return finalChar
